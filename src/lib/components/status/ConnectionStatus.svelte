<script lang="ts">
  ...
  let { status = "online", label, showLabel = false, theme, customColor }: { status?: "online"|"offline"|"connecting"; label?: string; showLabel?: boolean; theme?: Theme; customColor?: string } = $props();
...
<div class="cs" data-theme={theme} style={theme === "custom" && customColor ? `--custom-color: ${customColor}` : ""} title={displayLabel}>
