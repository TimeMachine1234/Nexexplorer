<script lang="ts">
  ...
  let { type = "empty", message, description, action, theme, customColor }: Props = $props();
...
<div class="empty-state empty-state--{type}" data-theme={theme} style={theme === "custom" && customColor ? `--custom-color: ${customColor}` : ""}>
