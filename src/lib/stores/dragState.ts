...
export function dragBegin(paths: string[], x: number, y: number) {
  dragState.set({ active: false, pending: true, paths, x, y, dropTarget: null, _startX: x, _startY: y });
}

export function dragMove(x: number, y: number) {
  dragState.update((s) => {
    if (!s.pending) return s;
    const dx = x - s._startX;
    const dy = y - s._startY;
    const activated = s.active || Math.sqrt(dx * dx + dy * dy) > 6;
    return { ...s, active: activated, x, y };
  });
}
