...
## 🧠 Lessons From The File Pilot Founder — Read This Before Writing Any Code

The creator of File Pilot gave a full technical talk explaining exactly how he built it. These are the most important lessons extracted directly from what he said and how they apply to your app built in Rust + Svelte + Tauri.
...
Tell Windsurf: "Preload the last opened folder, drives list, and icon set during Tauri app initialization before the window is shown to the user."
...
Rust's Vec already does this when you pre-allocate with capacity. Tell Windsurf:
...
- This is already in your roadmap as "Full Windows context menu" — now you know why it needs careful threading
...
When a user clicks something, don't do the work immediately in the click handler. Queue it.
...
// RIGHT — invoke is already async in Tauri, but make sure
// the UI updates happen in a tick, never synchronously block
async function handleClick() {
...
He resists adding options even when users beg for them. He said it "quickly becomes a nightmare."
...
**What this means for you:**
Your $9 price is good. Do not lower it out of fear. If the quality is there people pay. He also said his original idea was $15 and he was talked into the current structure. You could potentially charge more if the AI features are genuinely impressive.
...
His Discord went from hundreds to thousands of users in months just from organic sharing.
...
### WHAT HE ADMITTED FILE PILOT IS MISSING (Your Opportunities)

From the talk, things he acknowledged as problems or gaps:

1. **No WebP preview support** — he admitted he cannot preview every file format and is considering a plugin system. You should support WebP from day one.
2. **Context menu freezes** — he said it will never be fixed. You can fix it with async loading.
3. **OpenGL driver bugs causing black screen** — you don't use OpenGL so this entire class of bug doesn't exist for you.
4. **Still in beta after 3.5 years** — slow release cycle. You have a clear 12-phase roadmap with ship date.
...
### PROMPT FOR WINDSURF — Use This For Performance Questions
